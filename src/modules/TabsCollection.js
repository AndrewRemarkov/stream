...
const rootSelected = '[data-js-tabs]'
...
class Tabs extends BaseComponent{
  ...
  onKeyDown = (event) => {
    ...
    const isMacOsHomeKey = metaKey && code === 'ArrowLeft'
    if (isMacOsHomeKey) {
    ...
    const isMacOsEndKey = metaKey && code === 'ArrowRight'
    if (isMacOsEndKey) {
      event.preventDefault()
      this.lastTab()
      return
...
class TabsCollection {
  ...
  init() {
    document.querySelectorAll(rootSelected).forEach((el) => {
      new Tabs(el)
    })
  }
}
